...
eff_params <- data.frame(
  ...
  thalf.eff.iv.obs = TRUE,   # effective t½ via IV MRT
  thalf.eff.obs    = TRUE,   # effective t½ via general MRT
...
as.data.frame(o_nca_eff) |>
  filter(PPTESTCD %in% c("thalf.eff.iv.obs", "thalf.eff.obs", "kel.obs")) |>
  select(Subject, PPTESTCD, PPORRES) |>
  arrange(Subject, PPTESTCD)
```
...
`auciv*` parameters compute AUC using IV-specific formulas. Unlike the general `auclast`/`aucinf.obs`, these treat the dose as a bolus at time 0 and are required when the first concentration (C0) is back-extrapolated rather than measured.

| Parameter | Meaning |
|---|---|
| `aucivlast` | AUC(0–last) using IV back-extrapolation to C0 |
| `aucivinf.obs` | AUC(0–∞) IV method, observed Clast |
| `aucivpbextlast` | % of `aucivinf.obs` beyond last sample (IV method) |
...
as.data.frame(o_nca_auciv) |>
  filter(PPTESTCD %in% c("aucivlast", "aucivinf.obs", "aucivpbextlast")) |>
  select(Subject, PPTESTCD, PPORRES) |>
  arrange(Subject, PPTESTCD)
```

> **When to use `auciv*` vs `auclast`/`aucinf.obs`:** For a true IV bolus where C0 is back-extrapolated (as in this example), `aucivlast` and `aucivinf.obs` give the more theoretically correct AUC. For practical purposes the numerical difference is usually small.
