...
> **`first.tmin`** works the same way for the minimum concentration: `options = list(first.tmin = TRUE)` returns the first occurrence of Cmin; `FALSE` returns the last.
...
| `kel.obs` | λz (from half-life fit, observed) |
| `kel.pred` | λz (from half-life fit, predicted Clast) |
| `kel.last` | λz (last window only) |
