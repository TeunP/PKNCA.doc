...
## Example 3: multi-value return

A function can return a **named list** to register multiple output parameters at once. Each name in the list becomes a separate row in the results.

```{r}
# Compute early and late AUC portions simultaneously
pk.calc.auc.split <- function(conc, time, auclast) {
  # Fraction of AUClast in first 4h vs. rest
  auc_early <- pk.calc.auc.last(conc[time <= 4], time[time <= 4])
  auc_late  <- auclast - auc_early
  list(
    auc.early = auc_early,
    auc.late  = auc_late,
    auc.early.fraction = auc_early / auclast
  )
}

add.interval.col(
  name        = "auc.early",
  FUN         = "pk.calc.auc.split",
  values      = c(FALSE, TRUE),
  unit_type   = "auc",
