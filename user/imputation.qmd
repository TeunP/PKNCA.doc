...
Appropriate after a single-dose study when no pre-dose drug was present. The
drug concentration at time 0 is assumed to be 0.
...
outside the current interval for the most recent observed concentration.
...
Provide a comma-separated string to apply methods in sequence. Each method
in the chain is tried in order; the first one that successfully adds a point
at the interval start wins.
...
PKNCA discovers imputation methods by scanning the search path for functions
whose names match this prefix, so the convention is mandatory.
