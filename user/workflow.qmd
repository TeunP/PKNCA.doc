...
| `min.hl.r.squared` | `0.9` | Minimum adjusted R² for λz acceptance |
| `adj.r.squared.factor` | `0.0001` | Penalty per extra point in adjusted R² (`adj.r² = r² - factor × n`) |
...
| `hl_method` | `"log-linear"` | Regression method: `"log-linear"` (OLS on log scale) or `"tobit"` (for data with BLQ) |
| `tobit_n_points_penalty` | `0` | Extra penalty per point in tobit method (0 = no penalty) |
...
| `conc.blq` | (list) | BLQ concentrations: `"drop"`, `"keep"`, or a number; can differ before/after Tmax via `list(before.tmax=, after.tmax=)` |
