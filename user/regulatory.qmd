...
The CDISC PP domain stores one parameter value per row, with the parameter code in `PPTESTCD`, the numeric result in `PPORRES`, and the standardized result in `PPSTRESN`. PKNCA's `as.data.frame()` output follows exactly this long-format convention.
